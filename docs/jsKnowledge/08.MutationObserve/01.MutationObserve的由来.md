...
注意：Mutation Events 的废弃并不代表 addEventListener 函数的废弃，Mutation Events仅表示与DOM交互的事件，而 addEventListener 还可以监听许多其他事件，如鼠标点击，滚动等等，具体可以参考：[Events - MDN](https://developer.mozilla.org/en-US/docs/Web/Events)
...
> 性能的迟缓主要来自于Muation Event注册后执行的回调函数属于同步执行代码，而函数内的复杂操作属于主要的执行损耗
